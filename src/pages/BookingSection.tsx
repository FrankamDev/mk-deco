...
export function BookingSection() {
  ...
  const [selectedCategory, setSelectedCategory] = useState<
    "consultation" | "reparation" | "finition"
  >("consultation");
  ...
  const services: ServiceItem[] = [
    {
      id: "c1",
      name: "Consultation Design & Prise de Mesures",
      duration: 45,
      price: 5000,
    ...
    {
      id: "c2",
      name: "Étude de Faisabilité & Devis Technique",
      duration: 90,
      price: 15000,
      category: "consultation",
    },
    {
      id: "c3",
      name: "Visite Technique de Chantier",
      duration: 120,
      price: 25000,
      category: "consultation",
    },
    {
      id: "b1",
      name: "Réparation Express Meuble",
      duration: 30,
      price: 3000,
      category: "reparation",
    },
    {
      id: "b2",
      name: "Intervention Structure & Finition Premium",
      duration: 45,
      price: 5000,
      category: "reparation",
    },
    {
      id: "s1",
      name: "Ponçage & Vernissage Sur Place",
      duration: 40,
      price: 8000,
      category: "finition",
    },
    {
      id: "s2",
      name: "Entretien Complet du Mobilier Bois",
      duration: 30,
      price: 6000,
      category: "finition",
    },
  ...
  const handleCategoryChange = (
    cat: "consultation" | "reparation" | "finition",
  ) => {
  ...
  const handleBookingSubmit = async (e: React.FormEvent) => {
    ...
    const textMessage = `✨ *NOUVELLE DEMANDE DE RÉSERVATION* ✨
-----------------------------------------
...
⚡ _Veuillez confirmer la disponibilité de ce créneau par retour de message._`;
  ...
  return (
    <>
      <section
        id="booking-section"
        className="relative min-h-screen py-24 bg-stone-50 dark:bg-slate-950 overflow-hidden flex items-center transition-colors duration-500"
      >
        <Navbar />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-12 relative z-10 w-full">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mb-16 items-end">
            <div className="lg:col-span-7 text-center lg:text-left">
              <span className="inline-flex items-center gap-2 px-3 py-1 bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800 text-[10px] font-mono font-bold tracking-widest text-slate-900 dark:text-lime-400 uppercase mb-4">
                <Hammer className="w-3 h-3 text-lime-500" /> Atelier de
            ...
            <div className="lg:col-span-5 text-center lg:text-right">
              <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 font-light max-w-sm mx-auto lg:ml-auto leading-relaxed">
                Sélectionnez votre prestation. Notre système synchronise vos
                préférences pour réserver l'artisan idéal.
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch max-w-6xl mx-auto">
            ...
            <div className="lg:col-span-4 flex flex-row lg:flex-col gap-2 overflow-x-auto scrollbar-none pb-2 lg:pb-0">
              {(["consultation", "reparation", "finition"] as const).map(
                (cat) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`w-8 h-8 flex items-center justify-center ${isSelected ? "text-lime-400" : "text-slate-400"}`}
                      >
                        ...
                        <span className="text-[10px] hidden lg:block font-light text-slate-400 dark:text-slate-500 mt-0.5">
                          {cat === "consultation"
                            ...
                            : cat === "reparation"
                              ...
                              : "Ponçage, vernis, entretien"}
                        </span>
                      </div>
                      {isSelected && (
                        <motion.div
                          layoutId="barIndicator"
                ...
                {status === "success" ? (
                  ...
                  >
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400 font-light max-w-sm mb-6 leading-relaxed">
                      Redirection vers l'assistant WhatsApp de l'atelier pour
                      finaliser le blocage horaire avec notre équipe.
                    </p>
                    <button
                  ...
                  >
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-12 gap-4 items-start">
                        ...
                        <div className="sm:col-span-7 space-y-1.5 text-left">
                          <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 flex items-center gap-1.5">
                            <Clock className="w-3 h-3 text-lime-500" /> Heures
                            disponibles (Live DB)
                          </label>
                          ...
                          {!selectedDate ? (
                            <div className="text-xs text-slate-400 italic p-3 bg-stone-50 dark:bg-slate-950/40 border border-dashed border-slate-200 dark:border-slate-800">
                              Veuillez choisir une date pour charger les heures
                              disponibles.
                            </div>
                          ) : slotsLoading ? (
                            ...
                            <div className="grid grid-cols-4 gap-2">
                              {[1, 2, 3, 4].map((n) => (
                                <div
                                  key={n}
                                  className="h-8 bg-stone-100 dark:bg-slate-950 animate-pulse border border-slate-200/40 dark:border-slate-900"
                                />
                              ))}
                            </div>
                          ) : (
                      ...
                      <div className="p-4 bg-stone-50 dark:bg-slate-950 border border-slate-200/60 dark:border-slate-800 flex justify-between items-center">
                        <div className="text-left">
                          ...
                          <span className="text-xs font-bold text-slate-950 dark:text-white">
                            {activeService.duration} min en atelier
                          </span>
                        </div>
                    ...
                    <div className="pt-4 border-t border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row justify-between items-center gap-4">
                      <div className="flex items-center gap-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                        <ShieldCheck className="w-4 h-4 text-lime-500" /> Aucun
                        prépaiement requis en ligne
                      </div>
                      <button
                        type="submit"
